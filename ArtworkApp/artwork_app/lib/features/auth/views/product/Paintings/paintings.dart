...
class Paintings extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Paintings', style: TextStyle(color: Colors.black)), // Başlık yazısını beyaz yap
      ),
      body: FutureBuilder(
        future: _getUserUid(), 
  ...
  Widget _buildProductList(String userUid) {
    return StreamBuilder(
      stream: FirebaseFirestore.instance.collection('paintingsProduct').orderBy('productName').snapshots(),
    ...
    return ListTile(
      title: Text(
        ...
        style: TextStyle(
          ...
          color: Colors.white, // Başlık yazısını beyaz yap
        ),
      ),
      subtitle: Column(
        ...
        children: [
          Text(
            ...
            style: TextStyle(
              fontSize: 16,
              color: Colors.white, // Açıklama yazısını beyaz yap
            ),
          ),
          Text(
            ...
            style: TextStyle(
              fontSize: 14,
              color: Colors.grey, // Artist yazısını gri yap
            ),
          ),
        ],
      ),
  ...
  void _addNewProduct(BuildContext context) async {
    ...
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('Add New Product', style: TextStyle(color: Colors.white)), // Başlık yazısını beyaz yap
        content: SingleChildScrollView(
          child: Column(
            children: [
              TextField(
                decoration: InputDecoration(labelText: 'Product Name', labelStyle: TextStyle(color: Colors.white)), // Giriş kutusu etiket yazısını beyaz yap
                style: TextStyle(color: Colors.white), // Giriş kutusu yazı rengini beyaz yap
                onChanged: (value) {
                  productName = value;
                },
              ),
              TextField(
                decoration: InputDecoration(labelText: 'Product Artist', labelStyle: TextStyle(color: Colors.white)), // Giriş kutusu etiket yazısını beyaz yap
                style: TextStyle(color: Colors.white), // Giriş kutusu yazı rengini beyaz yap
                onChanged: (value) {
                  productArtist = value;
                },
        ...
        actions: [
          TextButton(
            ...
            child: Text('Cancel', style: TextStyle(color: Colors.white)), // Buton yazısını beyaz yap
          ),
          TextButton(
            onPressed: () {
              if (productName.isNotEmpty && productArtist.isNotEmpty) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                  content: Text('Please enter product name and artist.', style: TextStyle(color: Colors.white)), // Snackbar yazısını beyaz yap
                ));
              }
            },
            child: Text('Add', style: TextStyle(color: Colors.white)), // Buton yazısını beyaz yap
          ),
        ],
      ),
    );
  ...
  void _uploadProductToFirestore(String productName, String productArtist) async {
    await FirebaseFirestore.instance.collection('paintingsProduct').add({
  ...
  void _deleteProduct(String productId) {
    FirebaseFirestore.instance.collection('paintingsProduct').doc(productId).delete()
      .then((value) {
